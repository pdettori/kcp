...
const (
	...
	clusterGroupLabel = "kcp.dev/group"
	ownedByLabel      = "kcp.dev/owned-by"
	pollInterval      = time.Minute
)

func (c *Controller) reconcile(ctx context.Context, deployment *appsv1.Deployment) error {
	log.Println("reconciling group deployment", deployment.Name)

	if deployment.Labels != nil && deployment.Labels[clusterGroupLabel] != "" {
		// This is a group deployment; get its leafs.
	...
	} else if deployment.Labels != nil && deployment.Labels[ownedByLabel] != "" {
...
func (c *Controller) createLeafs(ctx context.Context, root *appsv1.Deployment) error {
	cls, err := c.clusterLister.List(labels.SelectorFromSet(labels.Set{clusterGroupLabel: root.Labels[clusterGroupLabel]}))
	if err != nil {
		return err
	}
	...
	if len(cls) == 1 {
		// nothing to group, just label Deployment for the only cluster.
		if root.Labels == nil {
	...
	// If there are >1 Clusters, create a virtual Deployment labeled/named for each Cluster with the same number of replicas
	for _, cl := range cls {
		vd := root.DeepCopy()
		...
		// make sure group label is removed in leafs or it will keep recursing
		delete(vd.Labels, clusterGroupLabel)
